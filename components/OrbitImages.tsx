...
export default function OrbitImages({
  ...
  const path = useMemo(() => {
    switch (shape) {
      ...
      case "custom":
        return (
          customPath ||
          generateCirclePath(designCenterX, designCenterY, radius)
        );
      default:
        return generateEllipsePath(
          designCenterX,
  ...
  const items = images.map((src, index) => (
    <div key={`${src}-${index}`} className="w-full h-full rounded-full overflow-hidden">
      <Image
        src={src}
        alt={`${altPrefix} ${index + 1}`}
        width={itemSize}
        height={itemSize}
        className="orbit-image rounded-full object-cover w-full h-full"
        draggable={false}
